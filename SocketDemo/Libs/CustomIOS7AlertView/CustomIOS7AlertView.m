...
#define kSpaceBetweenOfButtons 20
...
{
//    DDLogCVerbose(@"Button Clicked! %ld, %ld", (long)buttonIndex, (long)[alertView tag]);
    [self close];
}
...
- (UIView *)createContainerView
{
    ...
    // 设置视图之间的分割线
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, dialogContainer.bounds.size.height - buttonHeight - 0.5, dialogContainer.bounds.size.width, 0.5)];
   // lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
    lineView.backgroundColor = [UIColor colorWithHexString:@"#4b4f53"];
    [dialogContainer addSubview:lineView];
    
    //设置自定义视图为圆角
    [dialogContainer.layer setMasksToBounds:YES];
    [dialogContainer.layer setCornerRadius:kCustomIOS7AlertViewCornerRadius];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    //除去button的间隔，button的宽度
    CGFloat buttonWidth = (container.bounds.size.width - (buttonTitles.count - 1)*0.5)/ [buttonTitles count];

    for (int i=0; i < [buttonTitles count]; i++) {

        UIButton *closeButton = [UIButton buttonWithType:UIButtonTypeCustom];

        [closeButton setFrame:CGRectMake(i * buttonWidth + i*0.5, container.bounds.size.height - buttonHeight, buttonWidth, buttonHeight)];

        [closeButton addTarget:self action:@selector(customIOS7dialogButtonTouchUpInside:) forControlEvents:UIControlEventTouchUpInside];
        [closeButton setTag:i];
        [closeButton setBackgroundColor:[UIColor colorWithHexString:@"#d5d5d5"]];
        ...
        // 设置视图之间的分割线
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(buttonWidth *(i+1), container.bounds.size.height - buttonHeight,0.5, buttonHeight)];
        lineView.backgroundColor = [UIColor colorWithHexString:@"#4b4f53"];
